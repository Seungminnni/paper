#!/usr/bin/env python3
"""
Server-side Smashed Data (Dictionary) Generation
서버 사이드 Smashed Data (사전) 생성
"""
...
def main():
    print("🖥️  Starting Server-side Smashed Data (Dictionary) Generation")
    print("=" * 60)

    # 데이터 로드 및 전처리
    print("📊 Loading data for server-side dictionary...")
    ...
    print(f"📈 Processing {len(data_A)} patient records for server-side dictionary")
    ...
    # Hidden states 추출
    print("\n🔍 Extracting hidden states for smashed data dictionary...")
    hidden_states_list = []
    ...
    hidden_states_concat = hidden_states_concat[:, 0, :].detach().numpy()

    # 벡터를 CSV 파일로 저장
    output_file = "Dictionary_smashed_data_layer2.csv"
    pd.DataFrame(hidden_states_concat).to_csv(output_file, index=False)

    print(f"\n🎉 Server-side dictionary generation completed!")
    print(f"✅ Smashed data dictionary saved to '{output_file}'")
